...
@RestController
@RequestMapping("/superuser/configurator/projects/{projectId}/config/form-models/{formModelId}/layouts/{layoutId}/cells/{cellId}/constraint")
@PreAuthorize("@userSecurityService.isSuperuser()")
public class FormLayoutCellConstraintController {

	private final RuleDAOService ruleDAOService;

	public FormLayoutCellConstraintController(final RuleDAOService ruleDAOService) {
	...
	public ResponseEntity<RuleConstraintDTO> getConstraint(
		...
		@PathVariable final UUID cellId
	) {
		return ResponseEntity.ok(ruleDAOService.getConstraint(projectId, cellId));
	}
	...
	public ResponseEntity<RuleConstraintDTO> saveConstraint(
		...
		return ResponseEntity.ok(ruleDAOService.saveConstraint(projectId, cellId,
			RuleConstraintOwnerType.FORM_LAYOUT_CELL, RuleConstraintConstraintType.DEFAULT, dto));
	}
}
