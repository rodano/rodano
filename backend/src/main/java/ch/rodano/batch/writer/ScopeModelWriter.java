...
public class ScopeModelWriter extends BaseWriter {
	...
	private static UUID resolveEventModelIdFlexible(final DSLContext tx, final UUID projectId,
	                                                final String currentScopeCode, final String refEventCode) {
		...
		final UUID existsSameScope = tx.select(EVENT_MODEL.EVENT_MODEL_ID).from(EVENT_MODEL)
			.where(EVENT_MODEL.PROJECT_ID.eq(projectId).and(EVENT_MODEL.EVENT_MODEL_ID.eq(sameScopeId)))
			.fetchOne(EVENT_MODEL.EVENT_MODEL_ID);
		if(existsSameScope != null) {
			return existsSameScope;
		}
		final List<UUID> matches = tx.select(EVENT_MODEL.EVENT_MODEL_ID).from(EVENT_MODEL)
	...
	private static void relateEventsFlexible(final DSLContext tx, final UUID projectId,
	                                         final String scopeCode, final UUID srcEventModelId,
	                                         final List<String> refCodes,
	                                         final org.jooq.TableField<?, UUID> refEventField,
	                                         final org.jooq.Table<?> relTable, final String relNameForLogs) {
		...
		for(String refCode : refCodes) {
			...
			if(refEventModelId == null) {
				LOGGER.warn("Scope '{}': {} event '{}' not found or ambiguous — skipping.", scopeCode, relNameForLogs, refCode);
				continue;
			}
	...
	private static <T extends Table<Record1<UUID>>> void linkMany(final DSLContext tx,
	                                                              final UUID projectId, final UUID scopeModelId,
	                                                              final String scopeCode, final List<String> codes,
		...
		for(String code : codes) {
			...
			final UUID targetId = tx.select(targetIdField).from(targetTable)
				.where(DSL.field("project_id", UUID.class).eq(projectId).and(targetCodeField.eq(code)))
				.fetchOne(targetIdField);
			if(targetId == null) {
				LOGGER.warn("Scope '{}': {} '{}' not found — skipping.", scopeCode, what, code);
				continue;
			}
	...
	private static void linkEventModel(final DSLContext tx, final UUID projectId, final UUID eventModelId,
	                                   final List<String> codes, final org.jooq.Table<?> targetTable,
	                                   final org.jooq.TableField<?, UUID> targetIdField,
	                                   final org.jooq.TableField<?, String> targetCodeField,
	                                   final org.jooq.Table<?> linkTable,
	                                   final org.jooq.TableField<?, UUID> linkTargetIdField, final String what) {
		if(codes == null || codes.isEmpty()) {
			return;
		}
		for(String code : codes) {
			...
			final UUID targetId = tx.select(targetIdField).from(targetTable)
				...
				.fetchOne(targetIdField);
			if(targetId == null) {
				LOGGER.warn("Event ctx {} '{}' not found — skipping.", what, code);
				continue;
			}
			tx.insertInto(linkTable)
				.set(DSL.field("project_id", UUID.class), projectId)
