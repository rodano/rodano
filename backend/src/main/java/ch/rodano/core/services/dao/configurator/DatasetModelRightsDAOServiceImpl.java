...
@Repository
public class DatasetModelRightsDAOServiceImpl implements DatasetModelRightsDAOService {

	private final DSLContext dslContext;

	public DatasetModelRightsDAOServiceImpl(final DSLContext dslContext) {
		this.dslContext = dslContext;
	}

	@Override
	@Transactional(readOnly = true)
	@Cacheable(value = "datasetModelRights", key = "#projectId.toString()")
	public Map<UUID, Map<UUID, EntityRightDTO>> getDatasetModelRights(final UUID projectId) {
		final var rows = dslContext
			.selectFrom(PROFILE_DATASET_MODEL_RIGHTS)
			.where(PROFILE_DATASET_MODEL_RIGHTS.PROJECT_ID.eq(projectId))
			.fetch();
		...
		for(final var row : rows) {
			result
				...
				.put(row.getDatasetModelId(), new EntityRightDTO(row.getCanRead(), row.getCanWrite()));
		}
		return result;
	}
	...
	@Override
	@Transactional
	@CacheEvict(value = "datasetModelRights", key = "#projectId.toString()")
	public void saveDatasetModelRights(final UUID projectId, final Map<UUID, Map<UUID, EntityRightDTO>> rights) {
		dslContext.deleteFrom(PROFILE_DATASET_MODEL_RIGHTS)
			.where(PROFILE_DATASET_MODEL_RIGHTS.PROJECT_ID.eq(projectId))
			.execute();
		...
		for(final var profileEntry : rights.entrySet()) {
			...
			for(final var entityEntry : profileEntry.getValue().entrySet()) {
				...
				dslContext.insertInto(PROFILE_DATASET_MODEL_RIGHTS)
					.set(PROFILE_DATASET_MODEL_RIGHTS.PROJECT_ID, projectId)
					.set(PROFILE_DATASET_MODEL_RIGHTS.PROFILE_ID, profileId)
					.set(PROFILE_DATASET_MODEL_RIGHTS.DATASET_MODEL_ID, entityEntry.getKey())
					.set(PROFILE_DATASET_MODEL_RIGHTS.CAN_READ, right.canRead())
					.set(PROFILE_DATASET_MODEL_RIGHTS.CAN_WRITE, right.canWrite())
					.execute();
			}
		}
	}
