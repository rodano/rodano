...
@Repository
public class ScopeModelRightsDAOServiceImpl implements ScopeModelRightsDAOService {

	private final DSLContext dslContext;

	public ScopeModelRightsDAOServiceImpl(final DSLContext dslContext) {
		this.dslContext = dslContext;
	}

	@Override
	@Transactional(readOnly = true)
	@Cacheable(value = "scopeModelRights", key = "#projectId.toString()")
	public Map<UUID, Map<UUID, EntityRightDTO>> getScopeModelRights(final UUID projectId) {
		final var rows = dslContext
			.selectFrom(PROFILE_SCOPE_MODEL_RIGHTS)
			.where(PROFILE_SCOPE_MODEL_RIGHTS.PROJECT_ID.eq(projectId))
			.fetch();
		...
		for(final var row : rows) {
			result
				...
				.put(row.getScopeModelId(), new EntityRightDTO(row.getCanRead(), row.getCanWrite()));
		}
		return result;
	}
	...
	@Override
	@Transactional
	@CacheEvict(value = "scopeModelRights", key = "#projectId.toString()")
	public void saveScopeModelRights(final UUID projectId, final Map<UUID, Map<UUID, EntityRightDTO>> rights) {
		dslContext.deleteFrom(PROFILE_SCOPE_MODEL_RIGHTS)
			.where(PROFILE_SCOPE_MODEL_RIGHTS.PROJECT_ID.eq(projectId))
			.execute();
		...
		for(final var profileEntry : rights.entrySet()) {
			...
			for(final var entityEntry : profileEntry.getValue().entrySet()) {
				...
				dslContext.insertInto(PROFILE_SCOPE_MODEL_RIGHTS)
					.set(PROFILE_SCOPE_MODEL_RIGHTS.PROJECT_ID, projectId)
					.set(PROFILE_SCOPE_MODEL_RIGHTS.PROFILE_ID, profileId)
					.set(PROFILE_SCOPE_MODEL_RIGHTS.SCOPE_MODEL_ID, entityEntry.getKey())
					.set(PROFILE_SCOPE_MODEL_RIGHTS.CAN_READ, right.canRead())
					.set(PROFILE_SCOPE_MODEL_RIGHTS.CAN_WRITE, right.canWrite())
					.execute();
			}
		}
	}
