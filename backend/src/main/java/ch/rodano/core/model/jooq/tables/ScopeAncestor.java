...
public class ScopeAncestor extends TableImpl<ScopeAncestorRecord> {
	...
	/**
	 * The column <code>scope_ancestor.depth</code>.
	 */
	public final TableField<ScopeAncestorRecord, Integer> DEPTH = createField(DSL.name("depth"), SQLDataType.INTEGER.defaultValue(DSL.field(DSL.raw("NULL"), SQLDataType.INTEGER)), this, "");
	...
	/**
	 * The column <code>scope_ancestor.ancestor_removed</code>.
	 */
	public final TableField<ScopeAncestorRecord, Boolean> ANCESTOR_REMOVED = createField(DSL.name("ancestor_removed"), SQLDataType.BOOLEAN.nullable(false).defaultValue(DSL.field(DSL.raw("0"), SQLDataType.BOOLEAN)), this, "");
	...
	private ScopeAncestor(Name alias, Table<ScopeAncestorRecord> aliased, Field<?>[] parameters, Condition where) {
		super(alias, null, aliased, parameters, DSL.comment(""), TableOptions.view("CREATE VIEW `scope_ancestor` AS with recursive recursive_scope_ancestor(`scope_fk`,`ancestor_fk`,`start_date`,`end_date`,`direct`,`depth`,`virtual`,`default_relation`,`ancestor_removed`) as (select `sr`.`scope_fk` AS `scope_fk`,`sr`.`parent_fk` AS `ancestor_fk`,`sr`.`start_date` AS `start_date`,`sr`.`end_date` AS `end_date`,1 AS `direct`,1 AS `depth`,`s`.`virtual` AS `virtual`,`sr`.`default` AS `default_relation`,`s`.`removed` AS `ancestor_removed` from (`rodano`.`scope_relation` `sr` join `rodano`.`scope` `s` on(`sr`.`parent_fk` = `s`.`pk`)) union all select `rsa`.`scope_fk` AS `scope_fk`,`sr`.`parent_fk` AS `parent_fk`,case when `sr`.`start_date` is null then `rsa`.`start_date` when `rsa`.`start_date` is null then `sr`.`start_date` else greatest(`sr`.`start_date`,`rsa`.`start_date`) end AS `start_date`,case when `sr`.`end_date` is null then `rsa`.`end_date` when `rsa`.`end_date` is null then `sr`.`end_date` else least(`sr`.`end_date`,`rsa`.`end_date`) end AS `end_date`,0 AS `direct`,`rsa`.`depth` + 1 AS `depth`,`s`.`virtual` <> 0 or `rsa`.`virtual` <> 0 AS `s.virtual or rsa.virtual`,`sr`.`default` <> 0 and `rsa`.`default_relation` <> 0 AS `sr.``default`` && rsa.``default_relation```,`s`.`removed` <> 0 or `rsa`.`ancestor_removed` <> 0 AS `ancestor_removed` from ((`recursive_scope_ancestor` `rsa` join `rodano`.`scope_relation` `sr` on(`sr`.`scope_fk` = `rsa`.`ancestor_fk`)) join `rodano`.`scope` `s` on(`sr`.`parent_fk` = `s`.`pk`)))select `recursive_scope_ancestor`.`scope_fk` AS `scope_fk`,`recursive_scope_ancestor`.`ancestor_fk` AS `ancestor_fk`,min(`recursive_scope_ancestor`.`start_date`) AS `start_date`,case when max(case when `recursive_scope_ancestor`.`end_date` is null then 1 else 0 end) = 0 then max(`recursive_scope_ancestor`.`end_date`) end AS `end_date`,bit_or(`recursive_scope_ancestor`.`direct`) AS `direct`,`recursive_scope_ancestor`.`depth` AS `depth`,bit_and(`recursive_scope_ancestor`.`virtual`) AS `virtual`,bit_or(`recursive_scope_ancestor`.`default_relation`) AS `default`,bit_and(`recursive_scope_ancestor`.`ancestor_removed`) AS `ancestor_removed` from `recursive_scope_ancestor` group by `recursive_scope_ancestor`.`scope_fk`,`recursive_scope_ancestor`.`ancestor_fk` order by `recursive_scope_ancestor`.`scope_fk`"), where);
	}
