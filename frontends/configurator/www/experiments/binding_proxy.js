...
(function() {
	...
	const triggers = {
		...
		isRegistered: function(object) {
			return triggers.objects.includes(object);
		},
		register: function(object, proxy) {
			triggers.objects.push(object);
			triggers.proxies.push(proxy);
		},
		getProxy: function(object) {
			return triggers.proxies[triggers.objects.indexOf(object)];
		}
	};
	...
	const handler = {
		set: function(object, property, value) {
			...
			const original_value = object[property];
			//set value in object
			object[property] = value;
			//trigger event
			bindbus.dispatch(new BusEventObjectChangeProperty(object, property, original_value, value));
		},
		apply: function(object, that, args) {
			console.log('apply', object, that, args);
			if(Array.isArray(object)) {
				const original_array = object.slice();
				//apply method
				const result = object.apply(that, args);
				//special stuff for array
				if(original_array.length !== object.length) {
					for(let i = original_array.length; i < object.length; i++) {
						bindbus.dispatch(new BusEventArrayAddElement(object, object[i]));
					}
					for(let i = object.length; i < original_array.length; i++) {
						bindbus.dispatch(new BusEventArrayRemoveElement(object, original_array[i], i));
					}
				}
				return result;
			}
			else {
				return object.apply(that, args);
			}
		}
	};

	function register(object) {
		if(!triggers.isRegistered(object)) {
			const proxy = new Proxy(object, handler);
			for(const property in object) {
				if(object.hasOwnProperty(property)) {
					if(Object.isObject(object[property]) || Array.isArray(object[property])) {
						proxy[property] = register(object[property]);
					}
				}
			}
			triggers.register(object, proxy);
			return proxy;
		}
		else {
			return triggers.getProxy(object);
		}
	}
	...
	HTMLSelectElement.prototype.bind = function(object, prop, to_model, to_ui) {
		...
		this.addEventListener('change', this.binding.mbinder, false);
		//update input when object is modified
		const proxy = register(last_relation.object, last_relation.property);
	...
	HTMLElement.prototype.bindText = function(object, property, modifier) {
		return this.bind(object, property, function(value) {this.textContent = modifier ? modifier.call(undefined, value) : (value || '');});
	};

	HTMLElement.prototype.bind = function(object, property, to_ui) {
		const proxy = register(object);
	...
	Node.prototype.autobind = function(scope) {
		const proxy = register(scope);
		this.querySelectorAll('[data-bind]').forEach(function(node) {
			const last_object = Object.getLastObjectInPath(scope, node.dataset.bind);
